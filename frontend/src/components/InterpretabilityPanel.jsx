...
───────────────────────────────────────────────────────── */
export const Formula = ({ label, formula, result, description, highlight }) => (
  <div style={{
    padding: '12px 16px',
    borderRadius: '6px',
...
───────────────────────────────────────────────────────── */
export const Section = ({ title, defaultOpen = true, children }) => {
...
/* ─────────────────────────────────────────────────────────
   Sub-components for granular layout
───────────────────────────────────────────────────────── */

export const ComputedMetricsBox = ({ tokens, confidence, classification }) => {
  const m = useMemo(() => {
...
export const TopTokenDriversBox = ({ tokens }) => {
  const m = useMemo(() => {
    if (!tokens) return null;
    const parsed = typeof tokens === 'string' ? JSON.parse(tokens) : tokens;
    return deriveMetrics(parsed);
...
export const InterpretationSummaryBox = ({ tokens, confidence, classification, defaultOpen = true }) => {
  const m = useMemo(() => {
    if (!tokens) return null;
    const parsed = typeof tokens === 'string' ? JSON.parse(tokens) : tokens;
    return deriveMetrics(parsed);
  ...
  return (
    <Section title="📝 Interpretation Summary" defaultOpen={defaultOpen}>
...
export const TokenBreakdownBox = ({ tokens }) => {
  const m = useMemo(() => {
    if (!tokens) return null;
    const parsed = typeof tokens === 'string' ? JSON.parse(tokens) : tokens;
    return deriveMetrics(parsed);
...
const InterpretabilityPanel = ({ tokens, confidence, classification, compact = false }) => {
  return (
    <div style={{ fontFamily: 'var(--font-sans)' }}>
      <ComputedMetricsBox tokens={tokens} confidence={confidence} classification={classification} />
      <TopTokenDriversBox tokens={tokens} />
      {!compact && <TokenBreakdownBox tokens={tokens} />}
      <InterpretationSummaryBox tokens={tokens} confidence={confidence} classification={classification} defaultOpen={!compact} />
    </div>
  );
};
